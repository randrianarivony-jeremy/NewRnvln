...
const UserProfile = () => {
  ...
  const { data, isLoading, isSuccess, isError, error } =
    ...
    useEffect(() => {
      ...
      if (myUserSuccess) {
        if (myUser.friendRequest.includes(userId)) setFriend("request");
        else {
          if (myUser.friendInvitation.includes(userId)) setFriend("invitation");
          else setFriend("none");
        }
      }
    }, [currentUser, myUserSuccess]);

    if (isLoading || myUserLoading) return <Loader />;
    if (isError) return <ErrorRender isError={isError} error={error} />;
    if (isSuccess && myUserSuccess && user) {
      return (
